...
**v1.47.0** — Python 3.11+. Runtime is the standard library (`pytest` is for development only).

```text
python main.py --version
# Secret Scanner 1.47.0
```
...
- GitHub composite action (`uses: Dryhawell/secret-scanner@v1.47.0`)
...
- GitHub Action input `baseline` (read-only hashed JSON; empty = auto-discover sidecar)
...
2. **Format patterns** — public prefixes and shapes (`AKIA…` / `ASIA…`, `ghp_`, `glpat-`,
   ...
   `CCIPAT_` / `CCIPRJ_`, `fio-u-`, `duffel_live_`, `vcp_` / `vck_`,
   `EZAK` / `EZTK`, `CLOJARS_`, `LTAI`, `gsk_`,
...
| Grafana Token (`glc_` / `glsa_`) | HIGH |
| Square Token (`sq0atp-` / `sq0csp-`) | HIGH |
| Databricks Token (`dapi`) | HIGH |
...
| CircleCI Token (`CCIPAT_` / `CCIPRJ_`) | HIGH |
| Frame.io Token (`fio-u-`) | HIGH |
| Duffel Token (`duffel_live_` / `duffel_test_`) | HIGH |
| Vercel Token (`vcp_` / `vck_`) | HIGH |
| EasyPost Token (`EZAK` / `EZTK`) | HIGH |
| Clojars Token (`CLOJARS_`) | HIGH |
| Alibaba Access Key ID (`LTAI`) | HIGH |
| Groq API Key (`gsk_`) | HIGH |
...
jobs:
  secret-scan:
    ...
    steps:
      ...
      - uses: Dryhawell/secret-scanner@v1.47.0
        with:
          include-hidden: true
...
`verbose` (default false; DEBUG per-file lines in `logs/secret_scanner.log`,
not the job log),
`format` (empty or `text` = masked text in the job log; `json` / `html` /
...
`baseline` (empty = auto-discover `.secret-scanner-baseline.json`;
workspace-relative `.json` only; never `--update-baseline`),
...
and history modes, `--since` deltas, `--stdin`, the GitHub composite action, file globs, `--quiet`, `--min-confidence`, `--list-patterns` / `--skip-pattern` / `--only-pattern`, `--max-file-size`, oversized skip counts, binary skip counts, GitHub Action SARIF upload, `--fail-on-severity`, GitHub Action `max-file-size`, GitHub Action `min-confidence`, GitHub Action `skip-pattern`, GitHub Action `only-pattern`, GitHub Action `glob` / `skip-glob`, GitHub Action `jobs`, GitHub Action `exclude`, GitHub Action `verbose`, GitHub Action `format` / `output`, GitHub Action `config` / `ignore-file`, GitHub Action `baseline`, parallel file scans, the localhost dashboard, the hook
...
- `--jobs` uses threads (`concurrent.futures`). CPython's GIL means regex
  ...
  single-threaded. The Action input `jobs` is the same knob (empty = default 1).
...
- `--verbose` writes DEBUG per-file lines to the log file. It does not print
  plaintext secrets and does not change the exit code. The Action input is
  the same knob (default false). Combined with `quiet`, the job log stays
  silent while the log file still records paths.
...
- The GitHub composite action scans the **caller workspace** after checkout.
  ...
  sidecar auto-discovery. `baseline` is read-only hashed JSON (never
  `--update-baseline`); empty keeps sidecar auto-discovery. A PR can still
  ship a policy file that skips rules — treat that as a review item, not a
  scanner bug.
...
cli/github_action.py    composite-action argv (env → CLI, --no-color, optional SARIF, fail-on-severity, max-file-size, min-confidence, skip-pattern, only-pattern, glob, skip-glob, exclude, jobs, verbose, format, output, config, ignore-file, baseline)
